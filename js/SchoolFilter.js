schoolFilter = function (map) {
	
	this.map = map;
    
	var schoolsControl = new SchoolsControl(map);
	map.controls[google.maps.ControlPosition.TOP_RIGHT].clear();
	map.controls[google.maps.ControlPosition.TOP_RIGHT].push(schoolsControl);

	var mapPSLegend = new MapPSLegend();
...
function SchoolsControl(map) {
	...
	selectSecondaryFeature = function(){
		...
		fieldset.appendChild(optionControl('radio', 'feature', 'boys', 'nearby-boys', 'feature', 'Boys', app.state.nearby.filterFeatureForType[app.state.nearby.type] ?  app.state.nearby.filterFeatureForType[app.state.nearby.type].name == 'boys': false, radioClick));
		fieldset.appendChild(optionControl('radio', 'feature', 'girls', 'nearby-girls', 'feature', 'Girls', app.state.nearby.filterFeatureForType[app.state.nearby.type] ?  app.state.nearby.filterFeatureForType[app.state.nearby.type].name == 'girls': false, radioClick));
	...
	schoolFiltersControl = function(){
		...
		div.appendChild(schoolTypeControl());
	...
	schoolTypeControl = function(){
		...
		selectList.addEventListener('change', nearbySchoolsFilterChange);
	...
	selectOption = function(value, text, selected){
	...
	optionControl = function(type, className, value, id, name, labeltext, checked, onClick){
		var div = document.createElement('div');
    ...
    nearbySchoolsFilterChange = function(){
	...
	controlUI.appendChild(nearbySchoolsHeaderControl());		
	...
	controlUI.appendChild(schoolFiltersControl());
	
//	var mapPSLegend = new MapPSLegend();
//	controlUI.appendChild(mapPSLegend);
